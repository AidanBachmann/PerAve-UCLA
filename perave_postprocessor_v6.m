...
if param.suppress_plots == 0
    ...
    subplot(2,2,1)
    % Plots
    semilogy(zpos,avgPower,'b'); % Plot average radiation power
    hold on
    semilogy(zpos,max(power'),color='r'); % Plot max radiation power
    if param.P0 < param.Psat % If in the exponential gain regime, fit gain length and compare to theory
...
if param.suppress_plots == 0
    subplot(2,2,2);
    hold on
    plot(zpos,etapT); % Plot efficiency along undulator
    plot(zpos,etapN);
    ...
    subplot(2,2,3);
    hold on
    plot(zpos,abs(bunch)); % Plot bunching factor
    %plot(zpos,abs(bf));
...
if param.suppress_plots == 0
    ...
    subplot(1,2,1);
    hold on
    plot(zpos,abs(radfield(:,1))); % Plot field magnitude
    ...
    subplot(1,2,2);
    hold on
    plot(zpos,atan2(imag(radfield(:,1)),real(radfield(:,1)))); % Plot field phase
    ...
    for ii = 1:param.Np
        ...
        scatter(zpos,thetap(:,1,ii)./(pi),'.'); % Plot particle phase
    end
...
plot(zpos,estimateTrapFraction(param,thetap));
legend('The Highway','My Way');
hold off
%%}
