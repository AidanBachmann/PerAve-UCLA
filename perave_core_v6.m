...
trapfrac = zeros(param.Nsnap,1); % Array to store fraction of trapped particles
...
if(param.itdp)
    ...
    for ij = 1:param.Nsnap-1  % takes Nsnap snapshots along length of undulator
        ...
        trapfrac(ij,1) = estimateTrapFraction(param,thetap,ij); % Trapped fraction at snapshot ij
        bf(ij) = mean(exp(1j.*thetap(ij,1,:))./gammap(ij,1,:),3); % Bunching factor defined in Murphy and Pellegrini, equation 8.20
        etapT(ij,1) = eta1D(param,trapfrac(ij,1)*sin(param.psir),zpos(ij)); % Theoretical relative energy loss
        etapN(ij,1) = trapfrac(ij,1)*(1/param.gamma0)*abs(param.gamma0 - mean(gammap(ij,1,:),3)); % Numerical relative energy loss
...
% Compute bunching and relative energy loss at last time step (this is not done in the loop above)
bunch(param.Nsnap) = (mean(exp(1j.*thetap(param.Nsnap,1,:)),3));
bf(param.Nsnap) = mean(exp(1j.*thetap(param.Nsnap,1,:))./gammap(param.Nsnap,1,:),3);
trapfrac(param.Nsnap,1) = estimateTrapFraction(param,thetap,param.Nsnap); % Trapped fraction at snapshot ij
etapT(param.Nsnap,1) = eta1D(param,trapfrac(param.Nsnap,1)*sin(param.psir),zpos(param.Nsnap)); % Theoretical relative energy loss
etapN(param.Nsnap,1) = trapfrac(param.Nsnap,1)*(1/param.gamma0)*abs(param.gamma0 - mean(gammap(param.Nsnap,1,:),3)); % Numerical relative energy loss
