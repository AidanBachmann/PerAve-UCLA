...
% Fomrat: phasespace --> thetaf,gammaf
   
gammar_sq=param.lambdau/(2*param.lambda0)*(1+kvalue^2);
if param.undulator_type == 0
    sc = besselj(0,param.chi) - besselj(1,param.chi); % Planar undulator
else
    sc = 1; % Helical undulator
end
% Euler method for field (for some reason the most accurate...)
newevalue=evalue-param.stepsize*(param.chi1*kvalue*sc*mean(exp(-1i*phasespace(:,1))./phasespace(:,2)));
...
k1gamma=param.stepsize*(param.chi2*(kvalue./phasespace(:,2)).*real(evalue*sc*exp(1i*phasespace(:,1))));

k2theta=param.stepsize*(param.ku*(1-(gammar_sq./(phasespace(:,2)+0.5*k1gamma).^2)));
k2gamma=param.stepsize*(param.chi2*(kvalue./(phasespace(:,2)+0.5*k1gamma)).*real(evalue*sc*exp(1i*(phasespace(:,1)+0.5*k1theta))));

k3theta=param.stepsize*(param.ku*(1-(gammar_sq./(phasespace(:,2)+0.5*k2gamma).^2)));
k3gamma=param.stepsize*(param.chi2*(kvalue./(phasespace(:,2)+0.5*k2gamma)).*real(evalue*sc*exp(1i*(phasespace(:,1)+0.5*k2theta))));

k4theta=param.stepsize*(param.ku*(1-(gammar_sq./(phasespace(:,2)+k3gamma).^2)));
k4gamma=param.stepsize*(param.chi2*(kvalue./(phasespace(:,2)+k3gamma)).*real(evalue*sc*exp(1i*(phasespace(:,1)+k3theta))));
